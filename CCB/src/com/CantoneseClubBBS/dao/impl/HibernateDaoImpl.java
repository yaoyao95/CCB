...
package com.CantoneseClubBBS.dao.impl;
...
public class HibernateDaoImpl implements HibernateDao {
	...
	public <T> List<T> findByPage(String hql, Integer startRow, Integer pageSize, Object... params) {
		...
		if (ObjectUtils.isEmpty(params)) {
			return getSession().createQuery(hql).setCacheable(true).setFirstResult(startRow).setMaxResults(pageSize)
					.list();
		} else {
			...
			return (List<T>) query.setFirstResult(startRow).setMaxResults(pageSize).list();
		}
	}
	...
	public <T> Iterator<T> iterate(String queryString) {
		return getSession().createQuery(queryString).list().iterator();
	}
	...
	public <T> List<T> findByPage(String queryString, PageModel pageModel, List<?> params) {
		...
		// -----------查询总记录条数
		// 获取查询语句hql的from后面的表名前后的字符的index，
		// 用于获取表名 eg: select count(*) from notice
