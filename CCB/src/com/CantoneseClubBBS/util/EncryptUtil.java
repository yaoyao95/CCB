...
public class EncryptUtil {
	/** 密钥是16位长度的byte[]进行Base64转换后得到的字符串 秘钥加工前：xbdxyys39527395273 其他时候用的 */
	public static String key = "eGJkeHl5czM5NTI3Mzk1Mjcz";
	/** 密钥是16位长度的byte[]进行Base64转换后得到的字符串 秘钥加工前：xbdxyysdbwxhljxA 储存进db时用的 */
	public static String dbkey = "eGJkeHl5c2Rid3hobGp4QQ==";

	public static final int DB_Input = 1;
	 ...
	 *            需要加密的消息字符串
	 * @param operate
	 *            加密数据的用途.未定义的操作则用默认的，相当于encrypt(String xmlStr)
	 * @return 加密后的字符串
	 */
	public static String encrypt(String xmlStr, int operate) {
		byte[] encrypt = null;
		...
		byte[] iv = new byte[8];
		if (operate == 1) {
			getKeyIV(EncryptUtil.dbkey, key, iv);
		} else {
			getKeyIV(EncryptUtil.key, key, iv);
		}
	 ...
	 *            需要解密的消息字符串
	 * @param operate
	 *            需要解密的数据的用途。未定义的操作则用默认的，相当于decrypt(String xmlStr)
	 * @return 解密后的字符串
	 * @throws Exception
	 */
	public static String decrypt(String xmlStr, int operate) throws Exception {
		// base64解码
		BASE64Decoder decoder = new BASE64Decoder();
		byte[] encBuf = null;
		try {
	...
	public static void main(String[] args) {
		...
		System.out.println(encrypt(pre, EncryptUtil.DB_Input));
	}
